...
class ChooseByQualifiedNameContributor : GoGotoContributorBase<GoNamedElement>(
    ...
    override fun processElementsWithName(
        ...
        if (!completePattern.contains(name)) return
        // remove line number if any
        val fqn = completePattern.substringBefore(':')
        val iLastPathSep = fqn.lastIndexOf('/')
        if (iLastPathSep <= 0) return
        val packageDot = fqn.indexOf('.', iLastPathSep + 1)
        if (packageDot < 0) return
        val importPath = fqn.substring(0, packageDot)
        val packages = GoPackageUtil.findByImportPath(importPath, parameters.project, null, ResolveState.initial())
        logger.debug { "processElementsWithName name=${name}, completePattern=$completePattern, importPath=$importPath, packages.size=${packages.size}" }
